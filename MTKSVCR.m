function [Bpro_cell,time,gamma]=MTKSVCR(xTrain,xTest,yTrain,yTest,X,Y,k1,k2,p,delta,cc,dd,rho,class)
%%% This is the demo of the function corresponding to the inner loop
% Function MTKSVCR
...
%  Objective of this document: Get the prediction value, training time and the optimal dual solution corresponding to each parameter
...
H=(H+H')/2;
[gamma]=quadprog(H,f,A,b,Aeq,beq,lb,ub,[],solver);
time=toc;
...
for i=1:T
    ...
    w0x{i,1}=tstKer1use*(Yall*gamma(1:LpnAll,1))-tstKer2use*(gamma((LpnAll+1):(LpnAll+L3All),1)-gamma((LpnAll+L3All+1):end,1));%% the task-common part of the testing sample in the i-th task
    wtx{i,1}=tstKer1{i,i}*(diag(y{i,1})*Alpha{i,1})-tstKer2{i,i}*(Beta{i,1}-BetaS{i,1});%% the task-specific part of the testing sample in the i-th task
